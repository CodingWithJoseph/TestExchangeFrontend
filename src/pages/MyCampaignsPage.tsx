...
export function MyCampaignsPage() {
  ...
  return (
    <div className="page-stack">
      ...
      {createdCampaign && <div className="campaign-created-banner"><CheckCircle2 size={18} /><div><strong>{createdCampaign} is live.</strong><p>Its contract is locked and the tester rewards are reserved.</p></div></div>}
      ...
      {isLoading ? <div className="empty-state"><p>Loading campaigns…</p></div> : <div className="campaigns-grid">
        {storedDrafts.map((draft) => <article className="campaign-card" key={draft.id}>
          ...
          <div className="campaign-details"><span><small>TESTER GOAL</small><strong>{draft.testerGoal}</strong></span><span><small>CREDITS TO RESERVE</small><strong>{draft.testerGoal * draft.creditsPerTester}</strong></span></div>
        ...
        {campaigns.map((campaign) => {
          ...
          return <article className="campaign-card" key={campaign.id}>
            ...
            <div className="campaign-details"><span><small>APPLICATIONS</small><strong>{campaignAssignments.length}</strong></span><span><small>CREDITS RESERVED</small><strong>{campaign.target_testers * campaign.reward_credits}</strong></span></div>
      ...
      <aside className="info-banner"><span className="info-icon">i</span><div><strong>Publishing reserves the full tester reward pool.</strong><p>Approval releases one tester’s reward from that reserved pool; it does not charge the campaign twice.</p></div><Link to="/console/available-tests">Earn more credits <ArrowRight size={15} /></Link></aside>
    </div>
  )
}
