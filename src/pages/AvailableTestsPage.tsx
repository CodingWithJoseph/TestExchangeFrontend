    ...
    <div className="page-stack">
      ...
      {filteredTests.length > 0 ? (
        <div className="tests-grid">
          {filteredTests.map((test) => <TestCard test={test} key={test.id} onStart={setSelected} />)}
        </div>
      ) : (
        ...
        <div className="modal-backdrop" role="presentation" onMouseDown={() => setSelected(null)}>
          <section className="modal" role="dialog" aria-modal="true" aria-labelledby="test-dialog-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="modal-actions">
              ...
              <button className="button button-dark">Accept test · +{selected.credits} credits</button>
            </div>
          </section>
        </div>
      )}
