  ...
  return (
    <div className="page-stack review-page">
      ...
      <header className="review-header">
        <div className="workspace-app"><span className="avatar large-avatar">{assignment.tester_profile.display_name.split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase()}</span><div><span className="page-eyebrow">SUBMISSION REVIEW · VERSION {submission.version}</span><h1>{assignment.tester_profile.display_name}</h1><p>@{assignment.tester_profile.username} · Submitted {formatDate(submission.submitted_at)} · {campaign.platform}</p></div></div>
        ...
        <aside className="review-sidebar">
          <section className="decision-card">
            ...
            {!review ? <>
              ...
              <label className={`decision-button ${mode === 'changes_requested' ? 'selected' : ''}`}><input type="radio" name="decision" checked={mode === 'changes_requested'} onChange={() => { setMode('changes_requested'); setNotes('') }} /><RotateCcw size={18} /><span><strong>Request a correction</strong><small>Tester reward remains undecided</small></span></label>
